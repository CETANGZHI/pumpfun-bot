...
import bs58 from 'bs58'; // Import bs58 as default
...
// Load IDL
const idl: anchor.Idl = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'idl.json'), 'utf8'));

// pump.fun program ID
const PUMPFUN_PROGRAM_ID_STR = '6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P';
let PUMPFUN_PROGRAM_ID: PublicKey;
try {
...
let MPL_TOKEN_METADATA_PROGRAM_ID: PublicKey;
try {
...
interface CreateTokenParams {
    ...
    symbol: string;
    uri: string;
    buyAmountSol: number; // Amount of SOL to buy after creation
}

export async function createAndBuyToken({
    connection,
    payer,
    name,
    symbol,
    ...
    const associatedUserTokenAccount = await getAssociatedTokenAddress(
        mint.publicKey,
        payer.publicKey,
        false, // disallowOwnerOffCurve
        TOKEN_PROGRAM_ID,
        ASSOCIATED_TOKEN_PROGRAM_ID
    );
